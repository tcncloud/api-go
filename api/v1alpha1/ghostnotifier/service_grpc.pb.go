...
func (c *ghostNotifierApiClient) ListNotifications(ctx context.Context, in *ListNotificationsReq, opts ...grpc.CallOption) (GhostNotifierApi_ListNotificationsClient, error) {
	stream, err := c.cc.NewStream(ctx, &GhostNotifierApi_ServiceDesc.Streams[0], GhostNotifierApi_ListNotifications_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &ghostNotifierApiListNotificationsClient{stream}
...
func _GhostNotifierApi_ListNotifications_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(GhostNotifierApiServer).ListNotifications(m, &ghostNotifierApiListNotificationsServer{stream})
}

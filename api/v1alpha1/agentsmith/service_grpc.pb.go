...
func (c *agentSmithClient) FollowAgent(ctx context.Context, in *FollowAgentReq, opts ...grpc.CallOption) (AgentSmith_FollowAgentClient, error) {
	stream, err := c.cc.NewStream(ctx, &AgentSmith_ServiceDesc.Streams[0], AgentSmith_FollowAgent_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &agentSmithFollowAgentClient{stream}
...
func _AgentSmith_FollowAgent_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AgentSmithServer).FollowAgent(m, &agentSmithFollowAgentServer{stream})
}

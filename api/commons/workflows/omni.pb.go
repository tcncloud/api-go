// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        (unknown)
// source: api/commons/workflows/omni.proto

package workflows

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type OmniNodeScrublistAction int32

const (
	OmniNodeScrublistAction_ADD    OmniNodeScrublistAction = 0
	OmniNodeScrublistAction_REMOVE OmniNodeScrublistAction = 1
)

// Enum value maps for OmniNodeScrublistAction.
var (
	OmniNodeScrublistAction_name = map[int32]string{
		0: "ADD",
		1: "REMOVE",
	}
	OmniNodeScrublistAction_value = map[string]int32{
		"ADD":    0,
		"REMOVE": 1,
	}
)

func (x OmniNodeScrublistAction) Enum() *OmniNodeScrublistAction {
	p := new(OmniNodeScrublistAction)
	*p = x
	return p
}

func (x OmniNodeScrublistAction) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OmniNodeScrublistAction) Descriptor() protoreflect.EnumDescriptor {
	return file_api_commons_workflows_omni_proto_enumTypes[0].Descriptor()
}

func (OmniNodeScrublistAction) Type() protoreflect.EnumType {
	return &file_api_commons_workflows_omni_proto_enumTypes[0]
}

func (x OmniNodeScrublistAction) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OmniNodeScrublistAction.Descriptor instead.
func (OmniNodeScrublistAction) EnumDescriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{0}
}

// OmniNodePrompt is the PoC all-in-one version to send a message (optionally displaying options) and store the user input
type OmniNodePrompt struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the base message to send
	Prompt string `protobuf:"bytes,1,opt,name=prompt,proto3" json:"prompt,omitempty"`
	// the name (key in payload) under which to store the user input
	StoreTo string `protobuf:"bytes,2,opt,name=store_to,json=storeTo,proto3" json:"store_to,omitempty"`
	// a list of options to display along with the prompt
	Options []string `protobuf:"bytes,3,rep,name=options,proto3" json:"options,omitempty"`
}

func (x *OmniNodePrompt) Reset() {
	*x = OmniNodePrompt{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodePrompt) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodePrompt) ProtoMessage() {}

func (x *OmniNodePrompt) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodePrompt.ProtoReflect.Descriptor instead.
func (*OmniNodePrompt) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{0}
}

func (x *OmniNodePrompt) GetPrompt() string {
	if x != nil {
		return x.Prompt
	}
	return ""
}

func (x *OmniNodePrompt) GetStoreTo() string {
	if x != nil {
		return x.StoreTo
	}
	return ""
}

func (x *OmniNodePrompt) GetOptions() []string {
	if x != nil {
		return x.Options
	}
	return nil
}

// OmniNodeSendMessage will send a message, displaying options if provided
type OmniNodeSendMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the base message to send
	Prompt string `protobuf:"bytes,1,opt,name=prompt,proto3" json:"prompt,omitempty"`
	// the list of options to select from as the correct response
	Options []string `protobuf:"bytes,2,rep,name=options,proto3" json:"options,omitempty"`
}

func (x *OmniNodeSendMessage) Reset() {
	*x = OmniNodeSendMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeSendMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeSendMessage) ProtoMessage() {}

func (x *OmniNodeSendMessage) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeSendMessage.ProtoReflect.Descriptor instead.
func (*OmniNodeSendMessage) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{1}
}

func (x *OmniNodeSendMessage) GetPrompt() string {
	if x != nil {
		return x.Prompt
	}
	return ""
}

func (x *OmniNodeSendMessage) GetOptions() []string {
	if x != nil {
		return x.Options
	}
	return nil
}

// OmniNodeUserInput will store the text value of an incoming message under a given key in the payload
type OmniNodeUserInput struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the name (key in payload) under which to store the input (e.g. "input_text")
	StoreId string `protobuf:"bytes,1,opt,name=store_id,json=storeId,proto3" json:"store_id,omitempty"`
}

func (x *OmniNodeUserInput) Reset() {
	*x = OmniNodeUserInput{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeUserInput) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeUserInput) ProtoMessage() {}

func (x *OmniNodeUserInput) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeUserInput.ProtoReflect.Descriptor instead.
func (*OmniNodeUserInput) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{2}
}

func (x *OmniNodeUserInput) GetStoreId() string {
	if x != nil {
		return x.StoreId
	}
	return ""
}

// OmniNodeBranching compares the input against a list of options
type OmniNodeBranching struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// key of data stored in the payload to compare against the options (likely references an OmniNodeUserInput.store_id)
	StoreId string `protobuf:"bytes,1,opt,name=store_id,json=storeId,proto3" json:"store_id,omitempty"`
	// the list of options to use for branching
	Options []string `protobuf:"bytes,2,rep,name=options,proto3" json:"options,omitempty"`
}

func (x *OmniNodeBranching) Reset() {
	*x = OmniNodeBranching{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeBranching) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeBranching) ProtoMessage() {}

func (x *OmniNodeBranching) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeBranching.ProtoReflect.Descriptor instead.
func (*OmniNodeBranching) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{3}
}

func (x *OmniNodeBranching) GetStoreId() string {
	if x != nil {
		return x.StoreId
	}
	return ""
}

func (x *OmniNodeBranching) GetOptions() []string {
	if x != nil {
		return x.Options
	}
	return nil
}

// OmniNodeSetSkill adds the given string as a skill on the coversation
type OmniNodeSetSkill struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// a single skill to add to the conversation (deprecated)
	//
	// Deprecated: Marked as deprecated in api/commons/workflows/omni.proto.
	Skill string `protobuf:"bytes,1,opt,name=skill,proto3" json:"skill,omitempty"`
	// list of skills to add to the conversation
	Skills []string `protobuf:"bytes,2,rep,name=skills,proto3" json:"skills,omitempty"`
}

func (x *OmniNodeSetSkill) Reset() {
	*x = OmniNodeSetSkill{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeSetSkill) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeSetSkill) ProtoMessage() {}

func (x *OmniNodeSetSkill) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeSetSkill.ProtoReflect.Descriptor instead.
func (*OmniNodeSetSkill) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{4}
}

// Deprecated: Marked as deprecated in api/commons/workflows/omni.proto.
func (x *OmniNodeSetSkill) GetSkill() string {
	if x != nil {
		return x.Skill
	}
	return ""
}

func (x *OmniNodeSetSkill) GetSkills() []string {
	if x != nil {
		return x.Skills
	}
	return nil
}

// OmniNodeToAgent removes the conversation from the flow (updates the status)
type OmniNodeToAgent struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *OmniNodeToAgent) Reset() {
	*x = OmniNodeToAgent{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeToAgent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeToAgent) ProtoMessage() {}

func (x *OmniNodeToAgent) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeToAgent.ProtoReflect.Descriptor instead.
func (*OmniNodeToAgent) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{5}
}

// OmniNodeToMatcher removes the conversation from the flow (updates the status)
type OmniNodeToMatcher struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *OmniNodeToMatcher) Reset() {
	*x = OmniNodeToMatcher{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeToMatcher) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeToMatcher) ProtoMessage() {}

func (x *OmniNodeToMatcher) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeToMatcher.ProtoReflect.Descriptor instead.
func (*OmniNodeToMatcher) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{6}
}

// OmniNodeError will send an error message to the user and log an error
type OmniNodeError struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// the error message to send/log
	Error string `protobuf:"bytes,1,opt,name=error,proto3" json:"error,omitempty"`
}

func (x *OmniNodeError) Reset() {
	*x = OmniNodeError{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeError) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeError) ProtoMessage() {}

func (x *OmniNodeError) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeError.ProtoReflect.Descriptor instead.
func (*OmniNodeError) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{7}
}

func (x *OmniNodeError) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

type OmniNodeWebhook struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url     string            `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	Method  string            `protobuf:"bytes,2,opt,name=method,proto3" json:"method,omitempty"`
	Body    string            `protobuf:"bytes,3,opt,name=body,proto3" json:"body,omitempty"`
	Headers map[string]string `protobuf:"bytes,4,rep,name=headers,proto3" json:"headers,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *OmniNodeWebhook) Reset() {
	*x = OmniNodeWebhook{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeWebhook) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeWebhook) ProtoMessage() {}

func (x *OmniNodeWebhook) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeWebhook.ProtoReflect.Descriptor instead.
func (*OmniNodeWebhook) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{8}
}

func (x *OmniNodeWebhook) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *OmniNodeWebhook) GetMethod() string {
	if x != nil {
		return x.Method
	}
	return ""
}

func (x *OmniNodeWebhook) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

func (x *OmniNodeWebhook) GetHeaders() map[string]string {
	if x != nil {
		return x.Headers
	}
	return nil
}

// take an action on a scrublist, either add or remove (other?)
type OmniNodeScrublist struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Action OmniNodeScrublistAction `protobuf:"varint,1,opt,name=action,proto3,enum=api.commons.workflows.OmniNodeScrublistAction" json:"action,omitempty"`
}

func (x *OmniNodeScrublist) Reset() {
	*x = OmniNodeScrublist{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeScrublist) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeScrublist) ProtoMessage() {}

func (x *OmniNodeScrublist) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeScrublist.ProtoReflect.Descriptor instead.
func (*OmniNodeScrublist) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{9}
}

func (x *OmniNodeScrublist) GetAction() OmniNodeScrublistAction {
	if x != nil {
		return x.Action
	}
	return OmniNodeScrublistAction_ADD
}

// close out the conversation
type OmniNodeEndConversation struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *OmniNodeEndConversation) Reset() {
	*x = OmniNodeEndConversation{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_commons_workflows_omni_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OmniNodeEndConversation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OmniNodeEndConversation) ProtoMessage() {}

func (x *OmniNodeEndConversation) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_workflows_omni_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OmniNodeEndConversation.ProtoReflect.Descriptor instead.
func (*OmniNodeEndConversation) Descriptor() ([]byte, []int) {
	return file_api_commons_workflows_omni_proto_rawDescGZIP(), []int{10}
}

var File_api_commons_workflows_omni_proto protoreflect.FileDescriptor

var file_api_commons_workflows_omni_proto_rawDesc = []byte{
	,
}

var (
	file_api_commons_workflows_omni_proto_rawDescOnce sync.Once
	file_api_commons_workflows_omni_proto_rawDescData = file_api_commons_workflows_omni_proto_rawDesc
)

func file_api_commons_workflows_omni_proto_rawDescGZIP() []byte {
	file_api_commons_workflows_omni_proto_rawDescOnce.Do(func() {
		file_api_commons_workflows_omni_proto_rawDescData = protoimpl.X.CompressGZIP(file_api_commons_workflows_omni_proto_rawDescData)
	})
	return file_api_commons_workflows_omni_proto_rawDescData
}

var file_api_commons_workflows_omni_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_api_commons_workflows_omni_proto_msgTypes = make([]protoimpl.MessageInfo, 12)
var file_api_commons_workflows_omni_proto_goTypes = []interface{}{
	(OmniNodeScrublistAction)(0),    // 0: api.commons.workflows.OmniNodeScrublistAction
	(*OmniNodePrompt)(nil),          // 1: api.commons.workflows.OmniNodePrompt
	(*OmniNodeSendMessage)(nil),     // 2: api.commons.workflows.OmniNodeSendMessage
	(*OmniNodeUserInput)(nil),       // 3: api.commons.workflows.OmniNodeUserInput
	(*OmniNodeBranching)(nil),       // 4: api.commons.workflows.OmniNodeBranching
	(*OmniNodeSetSkill)(nil),        // 5: api.commons.workflows.OmniNodeSetSkill
	(*OmniNodeToAgent)(nil),         // 6: api.commons.workflows.OmniNodeToAgent
	(*OmniNodeToMatcher)(nil),       // 7: api.commons.workflows.OmniNodeToMatcher
	(*OmniNodeError)(nil),           // 8: api.commons.workflows.OmniNodeError
	(*OmniNodeWebhook)(nil),         // 9: api.commons.workflows.OmniNodeWebhook
	(*OmniNodeScrublist)(nil),       // 10: api.commons.workflows.OmniNodeScrublist
	(*OmniNodeEndConversation)(nil), // 11: api.commons.workflows.OmniNodeEndConversation
	nil,                             // 12: api.commons.workflows.OmniNodeWebhook.HeadersEntry
}
var file_api_commons_workflows_omni_proto_depIdxs = []int32{
	12, // 0: api.commons.workflows.OmniNodeWebhook.headers:type_name -> api.commons.workflows.OmniNodeWebhook.HeadersEntry
	0,  // 1: api.commons.workflows.OmniNodeScrublist.action:type_name -> api.commons.workflows.OmniNodeScrublistAction
	2,  // [2:2] is the sub-list for method output_type
	2,  // [2:2] is the sub-list for method input_type
	2,  // [2:2] is the sub-list for extension type_name
	2,  // [2:2] is the sub-list for extension extendee
	0,  // [0:2] is the sub-list for field type_name
}

func init() { file_api_commons_workflows_omni_proto_init() }
func file_api_commons_workflows_omni_proto_init() {
	if File_api_commons_workflows_omni_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_api_commons_workflows_omni_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodePrompt); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeSendMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeUserInput); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeBranching); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeSetSkill); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeToAgent); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeToMatcher); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeError); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeWebhook); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeScrublist); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_commons_workflows_omni_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OmniNodeEndConversation); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_commons_workflows_omni_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   12,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_api_commons_workflows_omni_proto_goTypes,
		DependencyIndexes: file_api_commons_workflows_omni_proto_depIdxs,
		EnumInfos:         file_api_commons_workflows_omni_proto_enumTypes,
		MessageInfos:      file_api_commons_workflows_omni_proto_msgTypes,
	}.Build()
	File_api_commons_workflows_omni_proto = out.File
	file_api_commons_workflows_omni_proto_rawDesc = nil
	file_api_commons_workflows_omni_proto_goTypes = nil
	file_api_commons_workflows_omni_proto_depIdxs = nil
}

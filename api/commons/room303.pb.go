// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: api/commons/room303.proto

package commons

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	wrapperspb "google.golang.org/protobuf/types/known/wrapperspb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RoomType int32

const (
	// Direct Message types - can be muliple users in a direct message
	RoomType_ROOM_TYPE_DIRECT RoomType = 0
	// Multiple User channel
	RoomType_ROOM_TYPE_MULTI RoomType = 1
	// System User channel - used for system notifications
	RoomType_ROOM_TYPE_SYSTEM RoomType = 2
	// Global system channel - used for global messages
	RoomType_ROOM_TYPE_GLOBAL_SYSTEM RoomType = 3
)

// Enum value maps for RoomType.
var (
	RoomType_name = map[int32]string{
		0: "ROOM_TYPE_DIRECT",
		1: "ROOM_TYPE_MULTI",
		2: "ROOM_TYPE_SYSTEM",
		3: "ROOM_TYPE_GLOBAL_SYSTEM",
	}
	RoomType_value = map[string]int32{
		"ROOM_TYPE_DIRECT":        0,
		"ROOM_TYPE_MULTI":         1,
		"ROOM_TYPE_SYSTEM":        2,
		"ROOM_TYPE_GLOBAL_SYSTEM": 3,
	}
)

func (x RoomType) Enum() *RoomType {
	p := new(RoomType)
	*p = x
	return p
}

func (x RoomType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RoomType) Descriptor() protoreflect.EnumDescriptor {
	return file_api_commons_room303_proto_enumTypes[0].Descriptor()
}

func (RoomType) Type() protoreflect.EnumType {
	return &file_api_commons_room303_proto_enumTypes[0]
}

func (x RoomType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RoomType.Descriptor instead.
func (RoomType) EnumDescriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{0}
}

type MessageStatus int32

const (
	MessageStatus_MESSAGE_STATUS_ACTIVE   MessageStatus = 0
	MessageStatus_MESSAGE_STATUS_EDITED   MessageStatus = 1
	MessageStatus_MESSAGE_STATUS_ARCHIVED MessageStatus = 2
)

// Enum value maps for MessageStatus.
var (
	MessageStatus_name = map[int32]string{
		0: "MESSAGE_STATUS_ACTIVE",
		1: "MESSAGE_STATUS_EDITED",
		2: "MESSAGE_STATUS_ARCHIVED",
	}
	MessageStatus_value = map[string]int32{
		"MESSAGE_STATUS_ACTIVE":   0,
		"MESSAGE_STATUS_EDITED":   1,
		"MESSAGE_STATUS_ARCHIVED": 2,
	}
)

func (x MessageStatus) Enum() *MessageStatus {
	p := new(MessageStatus)
	*p = x
	return p
}

func (x MessageStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MessageStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_api_commons_room303_proto_enumTypes[1].Descriptor()
}

func (MessageStatus) Type() protoreflect.EnumType {
	return &file_api_commons_room303_proto_enumTypes[1]
}

func (x MessageStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MessageStatus.Descriptor instead.
func (MessageStatus) EnumDescriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{1}
}

type RoomStatus int32

const (
	RoomStatus_ROOM_STATUS_ACTIVE   RoomStatus = 0
	RoomStatus_ROOM_STATUS_ARCHIVED RoomStatus = 1
	RoomStatus_ROOM_STATUS_DELETED  RoomStatus = 2
)

// Enum value maps for RoomStatus.
var (
	RoomStatus_name = map[int32]string{
		0: "ROOM_STATUS_ACTIVE",
		1: "ROOM_STATUS_ARCHIVED",
		2: "ROOM_STATUS_DELETED",
	}
	RoomStatus_value = map[string]int32{
		"ROOM_STATUS_ACTIVE":   0,
		"ROOM_STATUS_ARCHIVED": 1,
		"ROOM_STATUS_DELETED":  2,
	}
)

func (x RoomStatus) Enum() *RoomStatus {
	p := new(RoomStatus)
	*p = x
	return p
}

func (x RoomStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RoomStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_api_commons_room303_proto_enumTypes[2].Descriptor()
}

func (RoomStatus) Type() protoreflect.EnumType {
	return &file_api_commons_room303_proto_enumTypes[2]
}

func (x RoomStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RoomStatus.Descriptor instead.
func (RoomStatus) EnumDescriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{2}
}

// ConfigPermissionEnum -
type ConfigPermissionEnum int32

const (
	ConfigPermissionEnum_LIMITED            ConfigPermissionEnum = 0
	ConfigPermissionEnum_ROOM303_MEMBER     ConfigPermissionEnum = 1
	ConfigPermissionEnum_ROOM303_SUPERVISOR ConfigPermissionEnum = 2
)

// Enum value maps for ConfigPermissionEnum.
var (
	ConfigPermissionEnum_name = map[int32]string{
		0: "LIMITED",
		1: "ROOM303_MEMBER",
		2: "ROOM303_SUPERVISOR",
	}
	ConfigPermissionEnum_value = map[string]int32{
		"LIMITED":            0,
		"ROOM303_MEMBER":     1,
		"ROOM303_SUPERVISOR": 2,
	}
)

func (x ConfigPermissionEnum) Enum() *ConfigPermissionEnum {
	p := new(ConfigPermissionEnum)
	*p = x
	return p
}

func (x ConfigPermissionEnum) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ConfigPermissionEnum) Descriptor() protoreflect.EnumDescriptor {
	return file_api_commons_room303_proto_enumTypes[3].Descriptor()
}

func (ConfigPermissionEnum) Type() protoreflect.EnumType {
	return &file_api_commons_room303_proto_enumTypes[3]
}

func (x ConfigPermissionEnum) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ConfigPermissionEnum.Descriptor instead.
func (ConfigPermissionEnum) EnumDescriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{3}
}

type UserSid struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// id of the user
	UserId string `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// name of the user
	FullName string `protobuf:"bytes,2,opt,name=full_name,json=fullName,proto3" json:"full_name,omitempty"`
	// name of the user
	DisplayName   string `protobuf:"bytes,3,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserSid) Reset() {
	*x = UserSid{}
	mi := &file_api_commons_room303_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserSid) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserSid) ProtoMessage() {}

func (x *UserSid) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserSid.ProtoReflect.Descriptor instead.
func (*UserSid) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{0}
}

func (x *UserSid) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *UserSid) GetFullName() string {
	if x != nil {
		return x.FullName
	}
	return ""
}

func (x *UserSid) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

// Entity used in the member api service for room303.
type Member struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// contains name and id of the member to add
	UserSid *UserSid `protobuf:"bytes,1,opt,name=user_sid,json=userSid,proto3" json:"user_sid,omitempty"`
	// contains the name and id of the person adding the member
	AddedBy *UserSid `protobuf:"bytes,2,opt,name=added_by,json=addedBy,proto3" json:"added_by,omitempty"`
	// Unix timestamp for the time added
	AddedAt *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=added_at,json=addedAt,proto3" json:"added_at,omitempty"`
	// Room Id for this member
	RoomId string `protobuf:"bytes,4,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	// admin rights in the room
	Admin         bool `protobuf:"varint,5,opt,name=admin,proto3" json:"admin,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Member) Reset() {
	*x = Member{}
	mi := &file_api_commons_room303_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Member) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Member) ProtoMessage() {}

func (x *Member) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Member.ProtoReflect.Descriptor instead.
func (*Member) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{1}
}

func (x *Member) GetUserSid() *UserSid {
	if x != nil {
		return x.UserSid
	}
	return nil
}

func (x *Member) GetAddedBy() *UserSid {
	if x != nil {
		return x.AddedBy
	}
	return nil
}

func (x *Member) GetAddedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.AddedAt
	}
	return nil
}

func (x *Member) GetRoomId() string {
	if x != nil {
		return x.RoomId
	}
	return ""
}

func (x *Member) GetAdmin() bool {
	if x != nil {
		return x.Admin
	}
	return false
}

type Room struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// organization id
	OrgId string `protobuf:"bytes,1,opt,name=org_id,json=orgId,proto3" json:"org_id,omitempty"`
	// TODO: WIP deprecate - unique room id
	RoomId string `protobuf:"bytes,2,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	// type - DIRECT, MULTI, SYSTEM
	Type RoomType `protobuf:"varint,4,opt,name=type,proto3,enum=api.commons.RoomType" json:"type,omitempty"`
	// unix timestamp for time created
	CreatedAt *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	// unix timestamp for time updated
	UpdatedAt *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	// status of the room
	Status RoomStatus `protobuf:"varint,7,opt,name=status,proto3,enum=api.commons.RoomStatus" json:"status,omitempty"`
	// Id's value will be a hash when room_type is DIRECT or the room name when MULTI.
	// Used in ui for routing when room type is not available.
	// Will replace room_id once it's fully deprecated
	Id string `protobuf:"bytes,8,opt,name=id,proto3" json:"id,omitempty"`
	// display name for the room which is replacing the field "name".
	DisplayName string `protobuf:"bytes,9,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	// room configuration details
	Config *RoomConfig `protobuf:"bytes,10,opt,name=config,proto3" json:"config,omitempty"`
	// Optional id of the permission group assigned to this room.
	PermissionGroupId *wrapperspb.StringValue `protobuf:"bytes,11,opt,name=permission_group_id,json=permissionGroupId,proto3" json:"permission_group_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *Room) Reset() {
	*x = Room{}
	mi := &file_api_commons_room303_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Room) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Room) ProtoMessage() {}

func (x *Room) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Room.ProtoReflect.Descriptor instead.
func (*Room) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{2}
}

func (x *Room) GetOrgId() string {
	if x != nil {
		return x.OrgId
	}
	return ""
}

func (x *Room) GetRoomId() string {
	if x != nil {
		return x.RoomId
	}
	return ""
}

func (x *Room) GetType() RoomType {
	if x != nil {
		return x.Type
	}
	return RoomType_ROOM_TYPE_DIRECT
}

func (x *Room) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Room) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

func (x *Room) GetStatus() RoomStatus {
	if x != nil {
		return x.Status
	}
	return RoomStatus_ROOM_STATUS_ACTIVE
}

func (x *Room) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Room) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *Room) GetConfig() *RoomConfig {
	if x != nil {
		return x.Config
	}
	return nil
}

func (x *Room) GetPermissionGroupId() *wrapperspb.StringValue {
	if x != nil {
		return x.PermissionGroupId
	}
	return nil
}

type Message struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	OrgId      string                 `protobuf:"bytes,1,opt,name=org_id,json=orgId,proto3" json:"org_id,omitempty"`
	MessageId  string                 `protobuf:"bytes,2,opt,name=message_id,json=messageId,proto3" json:"message_id,omitempty"`
	RoomId     string                 `protobuf:"bytes,3,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	FromUser   *UserSid               `protobuf:"bytes,4,opt,name=from_user,json=fromUser,proto3" json:"from_user,omitempty"`
	Status     MessageStatus          `protobuf:"varint,5,opt,name=status,proto3,enum=api.commons.MessageStatus" json:"status,omitempty"`
	ReceivedAt *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=received_at,json=receivedAt,proto3" json:"received_at,omitempty"`
	UpdatedAt  *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	Payload    string                 `protobuf:"bytes,8,opt,name=payload,proto3" json:"payload,omitempty"`
	Unread     bool                   `protobuf:"varint,9,opt,name=unread,proto3" json:"unread,omitempty"`
	// nonce  is set by UI and used to tell if the message coming through the message update stream was created by that client.
	// this is so when the user is logged in on multiple clients (different device, tabs, browser) they can see their updated message on those clients.
	Nonce         string `protobuf:"bytes,10,opt,name=nonce,proto3" json:"nonce,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Message) Reset() {
	*x = Message{}
	mi := &file_api_commons_room303_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Message) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Message) ProtoMessage() {}

func (x *Message) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Message.ProtoReflect.Descriptor instead.
func (*Message) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{3}
}

func (x *Message) GetOrgId() string {
	if x != nil {
		return x.OrgId
	}
	return ""
}

func (x *Message) GetMessageId() string {
	if x != nil {
		return x.MessageId
	}
	return ""
}

func (x *Message) GetRoomId() string {
	if x != nil {
		return x.RoomId
	}
	return ""
}

func (x *Message) GetFromUser() *UserSid {
	if x != nil {
		return x.FromUser
	}
	return nil
}

func (x *Message) GetStatus() MessageStatus {
	if x != nil {
		return x.Status
	}
	return MessageStatus_MESSAGE_STATUS_ACTIVE
}

func (x *Message) GetReceivedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.ReceivedAt
	}
	return nil
}

func (x *Message) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

func (x *Message) GetPayload() string {
	if x != nil {
		return x.Payload
	}
	return ""
}

func (x *Message) GetUnread() bool {
	if x != nil {
		return x.Unread
	}
	return false
}

func (x *Message) GetNonce() string {
	if x != nil {
		return x.Nonce
	}
	return ""
}

type MessageStat struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	RoomId         string                 `protobuf:"bytes,1,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	UnreadMessages int32                  `protobuf:"varint,2,opt,name=unread_messages,json=unreadMessages,proto3" json:"unread_messages,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MessageStat) Reset() {
	*x = MessageStat{}
	mi := &file_api_commons_room303_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MessageStat) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MessageStat) ProtoMessage() {}

func (x *MessageStat) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MessageStat.ProtoReflect.Descriptor instead.
func (*MessageStat) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{4}
}

func (x *MessageStat) GetRoomId() string {
	if x != nil {
		return x.RoomId
	}
	return ""
}

func (x *MessageStat) GetUnreadMessages() int32 {
	if x != nil {
		return x.UnreadMessages
	}
	return 0
}

// global configuration settings
type GlobalConfig struct {
	state                      protoimpl.MessageState `protogen:"open.v1"`
	CreateRoom                 ConfigPermissionEnum   `protobuf:"varint,1,opt,name=create_room,json=createRoom,proto3,enum=api.commons.ConfigPermissionEnum" json:"create_room,omitempty"`
	JoinExistingRoom           ConfigPermissionEnum   `protobuf:"varint,2,opt,name=join_existing_room,json=joinExistingRoom,proto3,enum=api.commons.ConfigPermissionEnum" json:"join_existing_room,omitempty"`
	SendMessageToSupervisor    ConfigPermissionEnum   `protobuf:"varint,3,opt,name=send_message_to_supervisor,json=sendMessageToSupervisor,proto3,enum=api.commons.ConfigPermissionEnum" json:"send_message_to_supervisor,omitempty"`
	SendMessageToNonSupervisor ConfigPermissionEnum   `protobuf:"varint,4,opt,name=send_message_to_non_supervisor,json=sendMessageToNonSupervisor,proto3,enum=api.commons.ConfigPermissionEnum" json:"send_message_to_non_supervisor,omitempty"`
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *GlobalConfig) Reset() {
	*x = GlobalConfig{}
	mi := &file_api_commons_room303_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GlobalConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GlobalConfig) ProtoMessage() {}

func (x *GlobalConfig) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GlobalConfig.ProtoReflect.Descriptor instead.
func (*GlobalConfig) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{5}
}

func (x *GlobalConfig) GetCreateRoom() ConfigPermissionEnum {
	if x != nil {
		return x.CreateRoom
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *GlobalConfig) GetJoinExistingRoom() ConfigPermissionEnum {
	if x != nil {
		return x.JoinExistingRoom
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *GlobalConfig) GetSendMessageToSupervisor() ConfigPermissionEnum {
	if x != nil {
		return x.SendMessageToSupervisor
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *GlobalConfig) GetSendMessageToNonSupervisor() ConfigPermissionEnum {
	if x != nil {
		return x.SendMessageToNonSupervisor
	}
	return ConfigPermissionEnum_LIMITED
}

// room confirmation settings
type RoomConfig struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	AddUser        ConfigPermissionEnum   `protobuf:"varint,1,opt,name=add_user,json=addUser,proto3,enum=api.commons.ConfigPermissionEnum" json:"add_user,omitempty"`
	RemoveUser     ConfigPermissionEnum   `protobuf:"varint,2,opt,name=remove_user,json=removeUser,proto3,enum=api.commons.ConfigPermissionEnum" json:"remove_user,omitempty"`
	PromoteToAdmin ConfigPermissionEnum   `protobuf:"varint,3,opt,name=promote_to_admin,json=promoteToAdmin,proto3,enum=api.commons.ConfigPermissionEnum" json:"promote_to_admin,omitempty"`
	ReadMessages   ConfigPermissionEnum   `protobuf:"varint,4,opt,name=read_messages,json=readMessages,proto3,enum=api.commons.ConfigPermissionEnum" json:"read_messages,omitempty"`
	SendMessage    ConfigPermissionEnum   `protobuf:"varint,5,opt,name=send_message,json=sendMessage,proto3,enum=api.commons.ConfigPermissionEnum" json:"send_message,omitempty"`
	ArchiveRoom    ConfigPermissionEnum   `protobuf:"varint,6,opt,name=archive_room,json=archiveRoom,proto3,enum=api.commons.ConfigPermissionEnum" json:"archive_room,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RoomConfig) Reset() {
	*x = RoomConfig{}
	mi := &file_api_commons_room303_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoomConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoomConfig) ProtoMessage() {}

func (x *RoomConfig) ProtoReflect() protoreflect.Message {
	mi := &file_api_commons_room303_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoomConfig.ProtoReflect.Descriptor instead.
func (*RoomConfig) Descriptor() ([]byte, []int) {
	return file_api_commons_room303_proto_rawDescGZIP(), []int{6}
}

func (x *RoomConfig) GetAddUser() ConfigPermissionEnum {
	if x != nil {
		return x.AddUser
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *RoomConfig) GetRemoveUser() ConfigPermissionEnum {
	if x != nil {
		return x.RemoveUser
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *RoomConfig) GetPromoteToAdmin() ConfigPermissionEnum {
	if x != nil {
		return x.PromoteToAdmin
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *RoomConfig) GetReadMessages() ConfigPermissionEnum {
	if x != nil {
		return x.ReadMessages
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *RoomConfig) GetSendMessage() ConfigPermissionEnum {
	if x != nil {
		return x.SendMessage
	}
	return ConfigPermissionEnum_LIMITED
}

func (x *RoomConfig) GetArchiveRoom() ConfigPermissionEnum {
	if x != nil {
		return x.ArchiveRoom
	}
	return ConfigPermissionEnum_LIMITED
}

var File_api_commons_room303_proto protoreflect.FileDescriptor

const file_api_commons_room303_proto_rawDesc = "" +
	"\n" +
	"\x19api/commons/room303.proto\x12\vapi.commons\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1egoogle/protobuf/wrappers.proto\"b\n" +
	"\aUserSid\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\tR\x06userId\x12\x1b\n" +
	"\tfull_name\x18\x02 \x01(\tR\bfullName\x12!\n" +
	"\fdisplay_name\x18\x03 \x01(\tR\vdisplayName\"\xd0\x01\n" +
	"\x06Member\x12/\n" +
	"\buser_sid\x18\x01 \x01(\v2\x14.api.commons.UserSidR\auserSid\x12/\n" +
	"\badded_by\x18\x02 \x01(\v2\x14.api.commons.UserSidR\aaddedBy\x125\n" +
	"\badded_at\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aaddedAt\x12\x17\n" +
	"\aroom_id\x18\x04 \x01(\tR\x06roomId\x12\x14\n" +
	"\x05admin\x18\x05 \x01(\bR\x05admin\"\xc0\x03\n" +
	"\x04Room\x12\x15\n" +
	"\x06org_id\x18\x01 \x01(\tR\x05orgId\x12\x17\n" +
	"\aroom_id\x18\x02 \x01(\tR\x06roomId\x12)\n" +
	"\x04type\x18\x04 \x01(\x0e2\x15.api.commons.RoomTypeR\x04type\x129\n" +
	"\n" +
	"created_at\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\x12/\n" +
	"\x06status\x18\a \x01(\x0e2\x17.api.commons.RoomStatusR\x06status\x12\x0e\n" +
	"\x02id\x18\b \x01(\tR\x02id\x12!\n" +
	"\fdisplay_name\x18\t \x01(\tR\vdisplayName\x12/\n" +
	"\x06config\x18\n" +
	" \x01(\v2\x17.api.commons.RoomConfigR\x06config\x12L\n" +
	"\x13permission_group_id\x18\v \x01(\v2\x1c.google.protobuf.StringValueR\x11permissionGroupIdJ\x04\b\x03\x10\x04\"\xff\x02\n" +
	"\aMessage\x12\x15\n" +
	"\x06org_id\x18\x01 \x01(\tR\x05orgId\x12\x1d\n" +
	"\n" +
	"message_id\x18\x02 \x01(\tR\tmessageId\x12\x17\n" +
	"\aroom_id\x18\x03 \x01(\tR\x06roomId\x121\n" +
	"\tfrom_user\x18\x04 \x01(\v2\x14.api.commons.UserSidR\bfromUser\x122\n" +
	"\x06status\x18\x05 \x01(\x0e2\x1a.api.commons.MessageStatusR\x06status\x12;\n" +
	"\vreceived_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"receivedAt\x129\n" +
	"\n" +
	"updated_at\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\x12\x18\n" +
	"\apayload\x18\b \x01(\tR\apayload\x12\x16\n" +
	"\x06unread\x18\t \x01(\bR\x06unread\x12\x14\n" +
	"\x05nonce\x18\n" +
	" \x01(\tR\x05nonce\"O\n" +
	"\vMessageStat\x12\x17\n" +
	"\aroom_id\x18\x01 \x01(\tR\x06roomId\x12'\n" +
	"\x0funread_messages\x18\x02 \x01(\x05R\x0eunreadMessages\"\xea\x02\n" +
	"\fGlobalConfig\x12B\n" +
	"\vcreate_room\x18\x01 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\n" +
	"createRoom\x12O\n" +
	"\x12join_existing_room\x18\x02 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\x10joinExistingRoom\x12^\n" +
	"\x1asend_message_to_supervisor\x18\x03 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\x17sendMessageToSupervisor\x12e\n" +
	"\x1esend_message_to_non_supervisor\x18\x04 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\x1asendMessageToNonSupervisor\"\xaf\x03\n" +
	"\n" +
	"RoomConfig\x12<\n" +
	"\badd_user\x18\x01 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\aaddUser\x12B\n" +
	"\vremove_user\x18\x02 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\n" +
	"removeUser\x12K\n" +
	"\x10promote_to_admin\x18\x03 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\x0epromoteToAdmin\x12F\n" +
	"\rread_messages\x18\x04 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\freadMessages\x12D\n" +
	"\fsend_message\x18\x05 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\vsendMessage\x12D\n" +
	"\farchive_room\x18\x06 \x01(\x0e2!.api.commons.ConfigPermissionEnumR\varchiveRoom*h\n" +
	"\bRoomType\x12\x14\n" +
	"\x10ROOM_TYPE_DIRECT\x10\x00\x12\x13\n" +
	"\x0fROOM_TYPE_MULTI\x10\x01\x12\x14\n" +
	"\x10ROOM_TYPE_SYSTEM\x10\x02\x12\x1b\n" +
	"\x17ROOM_TYPE_GLOBAL_SYSTEM\x10\x03*b\n" +
	"\rMessageStatus\x12\x19\n" +
	"\x15MESSAGE_STATUS_ACTIVE\x10\x00\x12\x19\n" +
	"\x15MESSAGE_STATUS_EDITED\x10\x01\x12\x1b\n" +
	"\x17MESSAGE_STATUS_ARCHIVED\x10\x02*W\n" +
	"\n" +
	"RoomStatus\x12\x16\n" +
	"\x12ROOM_STATUS_ACTIVE\x10\x00\x12\x18\n" +
	"\x14ROOM_STATUS_ARCHIVED\x10\x01\x12\x17\n" +
	"\x13ROOM_STATUS_DELETED\x10\x02*O\n" +
	"\x14ConfigPermissionEnum\x12\v\n" +
	"\aLIMITED\x10\x00\x12\x12\n" +
	"\x0eROOM303_MEMBER\x10\x01\x12\x16\n" +
	"\x12ROOM303_SUPERVISOR\x10\x02B\x94\x01\n" +
	"\x0fcom.api.commonsB\fRoom303ProtoP\x01Z&github.com/tcncloud/api-go/api/commons\xa2\x02\x03ACX\xaa\x02\vApi.Commons\xca\x02\vApi\\Commons\xe2\x02\x17Api\\Commons\\GPBMetadata\xea\x02\fApi::Commonsb\x06proto3"

var (
	file_api_commons_room303_proto_rawDescOnce sync.Once
	file_api_commons_room303_proto_rawDescData []byte
)

func file_api_commons_room303_proto_rawDescGZIP() []byte {
	file_api_commons_room303_proto_rawDescOnce.Do(func() {
		file_api_commons_room303_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_api_commons_room303_proto_rawDesc), len(file_api_commons_room303_proto_rawDesc)))
	})
	return file_api_commons_room303_proto_rawDescData
}

var file_api_commons_room303_proto_enumTypes = make([]protoimpl.EnumInfo, 4)
var file_api_commons_room303_proto_msgTypes = make([]protoimpl.MessageInfo, 7)
var file_api_commons_room303_proto_goTypes = []any{
	(RoomType)(0),                  // 0: api.commons.RoomType
	(MessageStatus)(0),             // 1: api.commons.MessageStatus
	(RoomStatus)(0),                // 2: api.commons.RoomStatus
	(ConfigPermissionEnum)(0),      // 3: api.commons.ConfigPermissionEnum
	(*UserSid)(nil),                // 4: api.commons.UserSid
	(*Member)(nil),                 // 5: api.commons.Member
	(*Room)(nil),                   // 6: api.commons.Room
	(*Message)(nil),                // 7: api.commons.Message
	(*MessageStat)(nil),            // 8: api.commons.MessageStat
	(*GlobalConfig)(nil),           // 9: api.commons.GlobalConfig
	(*RoomConfig)(nil),             // 10: api.commons.RoomConfig
	(*timestamppb.Timestamp)(nil),  // 11: google.protobuf.Timestamp
	(*wrapperspb.StringValue)(nil), // 12: google.protobuf.StringValue
}
var file_api_commons_room303_proto_depIdxs = []int32{
	4,  // 0: api.commons.Member.user_sid:type_name -> api.commons.UserSid
	4,  // 1: api.commons.Member.added_by:type_name -> api.commons.UserSid
	11, // 2: api.commons.Member.added_at:type_name -> google.protobuf.Timestamp
	0,  // 3: api.commons.Room.type:type_name -> api.commons.RoomType
	11, // 4: api.commons.Room.created_at:type_name -> google.protobuf.Timestamp
	11, // 5: api.commons.Room.updated_at:type_name -> google.protobuf.Timestamp
	2,  // 6: api.commons.Room.status:type_name -> api.commons.RoomStatus
	10, // 7: api.commons.Room.config:type_name -> api.commons.RoomConfig
	12, // 8: api.commons.Room.permission_group_id:type_name -> google.protobuf.StringValue
	4,  // 9: api.commons.Message.from_user:type_name -> api.commons.UserSid
	1,  // 10: api.commons.Message.status:type_name -> api.commons.MessageStatus
	11, // 11: api.commons.Message.received_at:type_name -> google.protobuf.Timestamp
	11, // 12: api.commons.Message.updated_at:type_name -> google.protobuf.Timestamp
	3,  // 13: api.commons.GlobalConfig.create_room:type_name -> api.commons.ConfigPermissionEnum
	3,  // 14: api.commons.GlobalConfig.join_existing_room:type_name -> api.commons.ConfigPermissionEnum
	3,  // 15: api.commons.GlobalConfig.send_message_to_supervisor:type_name -> api.commons.ConfigPermissionEnum
	3,  // 16: api.commons.GlobalConfig.send_message_to_non_supervisor:type_name -> api.commons.ConfigPermissionEnum
	3,  // 17: api.commons.RoomConfig.add_user:type_name -> api.commons.ConfigPermissionEnum
	3,  // 18: api.commons.RoomConfig.remove_user:type_name -> api.commons.ConfigPermissionEnum
	3,  // 19: api.commons.RoomConfig.promote_to_admin:type_name -> api.commons.ConfigPermissionEnum
	3,  // 20: api.commons.RoomConfig.read_messages:type_name -> api.commons.ConfigPermissionEnum
	3,  // 21: api.commons.RoomConfig.send_message:type_name -> api.commons.ConfigPermissionEnum
	3,  // 22: api.commons.RoomConfig.archive_room:type_name -> api.commons.ConfigPermissionEnum
	23, // [23:23] is the sub-list for method output_type
	23, // [23:23] is the sub-list for method input_type
	23, // [23:23] is the sub-list for extension type_name
	23, // [23:23] is the sub-list for extension extendee
	0,  // [0:23] is the sub-list for field type_name
}

func init() { file_api_commons_room303_proto_init() }
func file_api_commons_room303_proto_init() {
	if File_api_commons_room303_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_api_commons_room303_proto_rawDesc), len(file_api_commons_room303_proto_rawDesc)),
			NumEnums:      4,
			NumMessages:   7,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_api_commons_room303_proto_goTypes,
		DependencyIndexes: file_api_commons_room303_proto_depIdxs,
		EnumInfos:         file_api_commons_room303_proto_enumTypes,
		MessageInfos:      file_api_commons_room303_proto_msgTypes,
	}.Build()
	File_api_commons_room303_proto = out.File
	file_api_commons_room303_proto_goTypes = nil
	file_api_commons_room303_proto_depIdxs = nil
}

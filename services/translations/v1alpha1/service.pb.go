// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.35.1
// 	protoc        (unknown)
// source: services/translations/v1alpha1/service.proto

package translationsv1alpha1

import (
	_ "github.com/tcncloud/api-go/annotations"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_services_translations_v1alpha1_service_proto protoreflect.FileDescriptor

var file_services_translations_v1alpha1_service_proto_rawDesc = []byte{
	,
}

var file_services_translations_v1alpha1_service_proto_goTypes = []any{
	(*TranslateTemplateRequest)(nil),       // 0: services.translations.v1alpha1.TranslateTemplateRequest
	(*ListTranslationsRequest)(nil),        // 1: services.translations.v1alpha1.ListTranslationsRequest
	(*ListLanguagesRequest)(nil),           // 2: services.translations.v1alpha1.ListLanguagesRequest
	(*ListContextsRequest)(nil),            // 3: services.translations.v1alpha1.ListContextsRequest
	(*CreateTranslationRequest)(nil),       // 4: services.translations.v1alpha1.CreateTranslationRequest
	(*UpdateTranslationRequest)(nil),       // 5: services.translations.v1alpha1.UpdateTranslationRequest
	(*TriggerLLMTranslationRequest)(nil),   // 6: services.translations.v1alpha1.TriggerLLMTranslationRequest
	(*TriggerLLMTranslationsRequest)(nil),  // 7: services.translations.v1alpha1.TriggerLLMTranslationsRequest
	(*SetSystemMessageRequest)(nil),        // 8: services.translations.v1alpha1.SetSystemMessageRequest
	(*GetSystemMessageRequest)(nil),        // 9: services.translations.v1alpha1.GetSystemMessageRequest
	(*TestSystemMessageRequest)(nil),       // 10: services.translations.v1alpha1.TestSystemMessageRequest
	(*EnableContextRequest)(nil),           // 11: services.translations.v1alpha1.EnableContextRequest
	(*DisableContextRequest)(nil),          // 12: services.translations.v1alpha1.DisableContextRequest
	(*BulkDeleteTranslationsRequest)(nil),  // 13: services.translations.v1alpha1.BulkDeleteTranslationsRequest
	(*TranslateTemplateResponse)(nil),      // 14: services.translations.v1alpha1.TranslateTemplateResponse
	(*ListTranslationsResponse)(nil),       // 15: services.translations.v1alpha1.ListTranslationsResponse
	(*ListLanguagesResponse)(nil),          // 16: services.translations.v1alpha1.ListLanguagesResponse
	(*ListContextsResponse)(nil),           // 17: services.translations.v1alpha1.ListContextsResponse
	(*CreateTranslationResponse)(nil),      // 18: services.translations.v1alpha1.CreateTranslationResponse
	(*UpdateTranslationResponse)(nil),      // 19: services.translations.v1alpha1.UpdateTranslationResponse
	(*TriggerLLMTranslationResponse)(nil),  // 20: services.translations.v1alpha1.TriggerLLMTranslationResponse
	(*TriggerLLMTranslationsResponse)(nil), // 21: services.translations.v1alpha1.TriggerLLMTranslationsResponse
	(*SetSystemMessageResponse)(nil),       // 22: services.translations.v1alpha1.SetSystemMessageResponse
	(*GetSystemMessageResponse)(nil),       // 23: services.translations.v1alpha1.GetSystemMessageResponse
	(*TestSystemMessageResponse)(nil),      // 24: services.translations.v1alpha1.TestSystemMessageResponse
	(*EnableContextResponse)(nil),          // 25: services.translations.v1alpha1.EnableContextResponse
	(*DisableContextResponse)(nil),         // 26: services.translations.v1alpha1.DisableContextResponse
	(*BulkDeleteTranslationsResponse)(nil), // 27: services.translations.v1alpha1.BulkDeleteTranslationsResponse
}
var file_services_translations_v1alpha1_service_proto_depIdxs = []int32{
	0,  // 0: services.translations.v1alpha1.TranslationsService.TranslateTemplate:input_type -> services.translations.v1alpha1.TranslateTemplateRequest
	1,  // 1: services.translations.v1alpha1.TranslationsService.ListTranslations:input_type -> services.translations.v1alpha1.ListTranslationsRequest
	2,  // 2: services.translations.v1alpha1.TranslationsService.ListLanguages:input_type -> services.translations.v1alpha1.ListLanguagesRequest
	3,  // 3: services.translations.v1alpha1.TranslationsService.ListContexts:input_type -> services.translations.v1alpha1.ListContextsRequest
	4,  // 4: services.translations.v1alpha1.TranslationsService.CreateTranslation:input_type -> services.translations.v1alpha1.CreateTranslationRequest
	5,  // 5: services.translations.v1alpha1.TranslationsService.UpdateTranslation:input_type -> services.translations.v1alpha1.UpdateTranslationRequest
	6,  // 6: services.translations.v1alpha1.TranslationsService.TriggerLLMTranslation:input_type -> services.translations.v1alpha1.TriggerLLMTranslationRequest
	7,  // 7: services.translations.v1alpha1.TranslationsService.TriggerLLMTranslations:input_type -> services.translations.v1alpha1.TriggerLLMTranslationsRequest
	8,  // 8: services.translations.v1alpha1.TranslationsService.SetSystemMessage:input_type -> services.translations.v1alpha1.SetSystemMessageRequest
	9,  // 9: services.translations.v1alpha1.TranslationsService.GetSystemMessage:input_type -> services.translations.v1alpha1.GetSystemMessageRequest
	10, // 10: services.translations.v1alpha1.TranslationsService.TestSystemMessage:input_type -> services.translations.v1alpha1.TestSystemMessageRequest
	11, // 11: services.translations.v1alpha1.TranslationsService.EnableContext:input_type -> services.translations.v1alpha1.EnableContextRequest
	12, // 12: services.translations.v1alpha1.TranslationsService.DisableContext:input_type -> services.translations.v1alpha1.DisableContextRequest
	13, // 13: services.translations.v1alpha1.TranslationsService.BulkDeleteTranslations:input_type -> services.translations.v1alpha1.BulkDeleteTranslationsRequest
	14, // 14: services.translations.v1alpha1.TranslationsService.TranslateTemplate:output_type -> services.translations.v1alpha1.TranslateTemplateResponse
	15, // 15: services.translations.v1alpha1.TranslationsService.ListTranslations:output_type -> services.translations.v1alpha1.ListTranslationsResponse
	16, // 16: services.translations.v1alpha1.TranslationsService.ListLanguages:output_type -> services.translations.v1alpha1.ListLanguagesResponse
	17, // 17: services.translations.v1alpha1.TranslationsService.ListContexts:output_type -> services.translations.v1alpha1.ListContextsResponse
	18, // 18: services.translations.v1alpha1.TranslationsService.CreateTranslation:output_type -> services.translations.v1alpha1.CreateTranslationResponse
	19, // 19: services.translations.v1alpha1.TranslationsService.UpdateTranslation:output_type -> services.translations.v1alpha1.UpdateTranslationResponse
	20, // 20: services.translations.v1alpha1.TranslationsService.TriggerLLMTranslation:output_type -> services.translations.v1alpha1.TriggerLLMTranslationResponse
	21, // 21: services.translations.v1alpha1.TranslationsService.TriggerLLMTranslations:output_type -> services.translations.v1alpha1.TriggerLLMTranslationsResponse
	22, // 22: services.translations.v1alpha1.TranslationsService.SetSystemMessage:output_type -> services.translations.v1alpha1.SetSystemMessageResponse
	23, // 23: services.translations.v1alpha1.TranslationsService.GetSystemMessage:output_type -> services.translations.v1alpha1.GetSystemMessageResponse
	24, // 24: services.translations.v1alpha1.TranslationsService.TestSystemMessage:output_type -> services.translations.v1alpha1.TestSystemMessageResponse
	25, // 25: services.translations.v1alpha1.TranslationsService.EnableContext:output_type -> services.translations.v1alpha1.EnableContextResponse
	26, // 26: services.translations.v1alpha1.TranslationsService.DisableContext:output_type -> services.translations.v1alpha1.DisableContextResponse
	27, // 27: services.translations.v1alpha1.TranslationsService.BulkDeleteTranslations:output_type -> services.translations.v1alpha1.BulkDeleteTranslationsResponse
	14, // [14:28] is the sub-list for method output_type
	0,  // [0:14] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_services_translations_v1alpha1_service_proto_init() }
func file_services_translations_v1alpha1_service_proto_init() {
	if File_services_translations_v1alpha1_service_proto != nil {
		return
	}
	file_services_translations_v1alpha1_entities_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_services_translations_v1alpha1_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_services_translations_v1alpha1_service_proto_goTypes,
		DependencyIndexes: file_services_translations_v1alpha1_service_proto_depIdxs,
	}.Build()
	File_services_translations_v1alpha1_service_proto = out.File
	file_services_translations_v1alpha1_service_proto_rawDesc = nil
	file_services_translations_v1alpha1_service_proto_goTypes = nil
	file_services_translations_v1alpha1_service_proto_depIdxs = nil
}
